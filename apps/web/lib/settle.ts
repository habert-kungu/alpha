...
import { POOLS, PoolType, effectiveCycle, poolLabel } from "./trading"
 ...
 * read: any active cycle at 100% effective progress is completed, its
 * investment closed, and its payout recorded.
 *
  ...
  for (const cycle of matured) {
    ...
    const claimed = await prisma.$transaction(async (tx) => {
      ...
      await tx.transaction.create({
        data: {
          ...
          currency: "USDT",
          status: "completed",
          note: `${poolLabel(pool, true)} Pool cycle completed`,
        },
      })
      return true
    })
    ...
    if (now - maturedAt(cycle, pool) < 24 * 60 * 60 * 1000) {
      ...
      void triggerNotification(CHANNELS.USER(cycle.userId), EVENTS.CYCLE_COMPLETED, {
        ...
        returnAmount: cycle.targetValue,
        message: `Your ${poolLabel(pool, true)} Pool cycle completed — $${cycle.targetValue.toLocaleString()} is now available.`,
      })
    }
  }
...
/** When the cycle hit 100%, given its baseline progress and the pool's term. */
function maturedAt(
  cycle: { progress: number; createdAt: Date; progressAt: Date | null },
  pool: string
): number {
  const stored = Math.max(0, Math.min(100, cycle.progress || 0))
  const term = (POOLS[pool as PoolType] ?? POOLS.weekly).durationDays * 24 * 60 * 60 * 1000
