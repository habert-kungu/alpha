...
function generateMarketData(startValue: number, targetValue: number, points: number = 48): { time: string; price: number }[] {
  ...
  for (let i = 0; i < points; i++) {
    ...
    currentValue = Math.max(startValue * 0.95, Math.min(targetValue * 1.02, currentValue))
...
export async function GET(request: NextRequest) {
  try {
    ...
    const chartData = activeCycles.map(cycle => {
      ...
      return {
        cycleId: cycle.id,
        pool: investments.find(i => i.id === cycle.investmentId)?.pool || 'weekly',
        data,
      }
    })
